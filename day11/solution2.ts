...
function newMonkey() : Monkey {
    let m : Monkey = {items: [], operation: "", arg1: "", arg2: "", test: Number(0), monkeyToTrue:0, monkeyToFalse:0, countItems: 0};
    return m;
}

function calcWorry(i : number, op: string, arg1: string, arg2: string, scale: number) : number{
...
function solve(content: string[]){    
    ...
    let scale = 1;
    monkeys.forEach(m=>{scale*=m.test});
    console.log(scale);
    ...
    for (let round = 0; round < 10000; round++){
        monkeys.forEach((m,mi)=>{
            m.items.forEach(i=>{
                m.countItems++;
                let worry = calcWorry(i, m.operation, m.arg1, m.arg2, scale);
        ...
        if ((round + 1) % 1000 === 0){
            console.log(round);
            monkeys.forEach((m,i)=>console.log("Monkey", i, "inspected items", m.countItems, "times."));
        }
    };
